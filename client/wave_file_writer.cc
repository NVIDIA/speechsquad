 ...
 * @brief Implementation of the WaveFileWriter class.
...
void writeToStream(std::ofstream &stream, const char *const bytes,
                   const size_t len) {
...
void WaveFileWriter::write(const std::string &filename, const int frequency,
                           const float *const data, const size_t numSamples) {
  if (!isLittleEndian()) {
